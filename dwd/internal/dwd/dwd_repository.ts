...
export class DwDRepository {
  ...
  private async closeData(warningids: string[]): Promise<number> {
    ...
    try {
      let SQL = `Update dwd.warnings SET loadenddate = CURRENT_TIMESTAMP WHERE warning_id NOT IN (${stringWarningIds}) AND loadenddate IS NULL`;

      if (warningids.length == 0) {
        SQL = `Update dwd.warnings SET loadenddate = CURRENT_TIMESTAMP WHERE loadenddate IS NULL`;
      }
  ...
  async checkData(warnings: IWarningModel[]): Promise<IWarningModel[]> {
    ...
    try {
      const result = await this.executeQuery(
        `SELECT warning_id FROM dwd.warnings WHERE warning_id = ANY($1)`,
        [values],
      );
    ...
    try {
      for (const warning of newwarnings) {
        ...
        await this.executeQuery(
          `INSERT INTO dwd.warnings (warning_id, warning_type, title, description, instruction, coordinates) VALUES ($1, $2, $3, $4, $5, ST_SetSRID(ST_GeomFromGeoJSON('${coordinates}'), 4326))`,
          [
            warning.id,
            warning.type,
            warning.title,
  ...
  async getClosedData(timestamp: number) {
    ...
    try {
      const result = await this.executeQuery(
        `SELECT warning_id FROM dwd.warnings WHERE loadenddate > TO_TIMESTAMP($1/1000)`,
        [timestamp],
      );
  ...
  private transformWarnings(row: QueryResultRow): IReturnSchema {
    ...
    return {
      ...
      title: row.title === "undefined" ? "Warnung" : row.title,
      area: coordinates,
      details: {
        description:
          row.description === "undefined" || row.description === "null"
            ...
            : row.description,
        instruction:
          row.instruction === "undefined" || row.description === "null"
            ? undefined
            : row.instruction,
      },
    };
  ...
  async getData(timestamp: number) {
    ...
    try {
      ...
      resultwarnings.rows.forEach((row: QueryResultRow) => {
        const warning = this.transformWarnings(row);
    ...
    return {
      description:
        row.description === "null" || row.description === "undefined"
          ...
          : row.description,
      instruction:
        row.instruction === "null" || row.description === "undefined"
          ? undefined
          : row.instruction,
    };
  }
